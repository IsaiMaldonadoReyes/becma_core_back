...
class PrenominaQueryService
{
    ...
    private function datosEncabezadoContpaq(Request $request)
    {
        try {
            ...
            $sql = "
                ...
                FROM (
                    SELECT
                        ...
                        MIN(emp.codigoempleado) AS codigoInicio,
                        MAX(emp.codigoempleado) AS codigoFin
                    FROM nom10002 p
                    INNER JOIN nom10023 tp
                        ON p.idtipoperiodo = tp.idtipoperiodo
                    INNER JOIN nom10001 emp
                        ON emp.idtipoperiodo = tp.idtipoperiodo
                    WHERE p.idperiodo = @idPeriodo
                    AND emp.idempleado BETWEEN @idEmpleadoInicial AND @idEmpleadoFinal
                    GROUP BY
                        p.ejercicio,
    ...
    private function datosEncabezadoExcedente(Request $request)
    {
        try {
            $idNominaGapeEmpresa = $request->id_nomina_gape_empresa;
            ...
            $sql = "
                ...
                Obligaciones AS (
                    SELECT
                        ...
                        1 AS orden
                    FROM nom10004
                    WHERE numeroconcepto IN (90, 4002, 8000, 8001, 8002)
                ),
                titulos AS (
                    SELECT * FROM (VALUES
                ...
                ;WITH
                    ...
                    Comision AS (
						SELECT
							q3.codigoempleado,
							(com.monto_asignado + ISNULL(t.monto,0)) * (com.fee / 100) AS total_comision
						FROM TotalesPorEmpleadoQ3 q3
						INNER JOIN totalCompensacion com
							ON q3.codigoempleado = com.codigoempleado
						LEFT JOIN TotalesPorEmpleadoObligacion t
							ON q3.codigoempleado = t.codigoempleado
					),
                    CostoTotal AS (
                        SELECT
                            codigoempleado,
                ...
                ;WITH MovimientosRetro AS (
                    ...
                    MovimientosO AS (
                        ...
                        WHERE
                            ...
                            AND con.numeroconcepto IN (90, 4002, 8000, 8001, 8002)
                        UNION ALL
                        SELECT idempleado, idperiodo, actual.idconcepto, valor, importetotal, con.descripcion, con.tipoconcepto, con.numeroconcepto
                        FROM Nom10008 AS actual
                        INNER JOIN nom10004 con
                    ...
                    TotalesCalculados AS (
                        SELECT
                            percepcion_bruta,
                            0 AS costo_social,
                            (percepcion_bruta) AS base,
                            (SELECT (fee/100) FROM nomGapeEmpresa) AS fee_porcentaje,
                            (percepcion_bruta) * (SELECT (fee/100) FROM nomGapeEmpresa) AS fee,
                            (percepcion_bruta) + ((percepcion_bruta) * (SELECT (fee/100) FROM nomGapeEmpresa)) AS subtotal,
                            ((percepcion_bruta) + ((percepcion_bruta) * (SELECT (fee/100) FROM nomGapeEmpresa))) * 0.16 AS iva,
                            ((percepcion_bruta) + ((percepcion_bruta) * (SELECT (fee/100) FROM nomGapeEmpresa))) * 1.16 AS total
                        FROM ResumenFinal
                    ) ' + '
                    SELECT *
    ...
    private function totalSueldoImss_02(Request $request)
    {
        try {
            $idNominaGapeCliente = $request->id_nomina_gape_cliente;
            $idNominaGapeEmpresa = $request->id_nomina_gape_empresa;
    ...
    private function totalSueldoImss_03(Request $request)
    {
        try {
            $idNominaGapeCliente = $request->id_nomina_gape_cliente;
            $idNominaGapeEmpresa = $request->id_nomina_gape_empresa;
                ...
                ;WITH MovimientosRetro AS (
                    ...
                    TotalesCalculados AS (
                        SELECT
                            percepcion_bruta,
                            0,
                            (percepcion_bruta + 0) AS base,
                            (SELECT (fee/100) FROM nomGapeEmpresa) AS fee_porcentaje,
                            (percepcion_bruta + 0) * (SELECT (fee/100) FROM nomGapeEmpresa) AS fee,
                            (percepcion_bruta + 0) + ((percepcion_bruta + 0) * (SELECT (fee/100) FROM nomGapeEmpresa)) AS subtotal,
                            ((percepcion_bruta + 0) + ((percepcion_bruta + 0) * (SELECT (fee/100) FROM nomGapeEmpresa))) * 0.16 AS iva,
                            ((percepcion_bruta + 0) + ((percepcion_bruta + 0) * (SELECT (fee/100) FROM nomGapeEmpresa))) * 1.16 AS total
                        FROM ResumenFinal
                    ) ' + '
                    SELECT *
                    FROM TotalesCalculados;
    ...
    private function totalSueldoImss_04(Request $request)
    {
        try {
            $idNominaGapeCliente = $request->id_nomina_gape_cliente;
            $idNominaGapeEmpresa = $request->id_nomina_gape_empresa;
    ...
    private function totalAsimilados_02(Request $request)
    {
        try {
            $idNominaGapeCliente = $request->id_nomina_gape_cliente;
            $idNominaGapeEmpresa = $request->id_nomina_gape_empresa;
                ...
                ;WITH MovimientosRetro AS (
                    ...
                    costoTotal AS (
                        SELECT
                            ...
                            ''percepcion'' AS concepto
                        FROM TotalesPorEmpleadoGeneralQ2
                        UNION ALL
                        SELECT
                            monto AS monto,
                            ''costo_social'' AS concepto
                        FROM TotalesPorEmpleadoObligacion
                    ), ' + '
                    totales AS (
                        SELECT
                            SUM(CASE WHEN concepto = ''percepcion'' THEN monto ELSE 0 END) AS percepcion_bruta,
                            SUM(CASE WHEN concepto = ''costo_social'' THEN monto ELSE 0 END) AS costo_social
                        FROM costoTotal
                    ) ' + '
                    SELECT
                        t.percepcion_bruta,
                        t.costo_social AS costo_social,
                        (t.percepcion_bruta + t.costo_social) AS base,
                        p.fee / 100.0 AS fee_porcentaje,
                        (t.percepcion_bruta + t.costo_social) * (p.fee / 100.0) AS fee,
                        (t.percepcion_bruta + t.costo_social) * (1 + p.fee / 100.0) AS subtotal,
                        (t.percepcion_bruta + t.costo_social) * (1 + p.fee / 100.0) * 0.16 AS iva,
                        (t.percepcion_bruta + t.costo_social) * (1 + p.fee / 100.0) * 1.16 AS total
                    FROM totales t
                    CROSS JOIN parametrizacion p;
                    ';
                    EXEC(@sql);
    ...
    private function totalAsimilados_03(Request $request)
    {
        try {
            $idNominaGapeCliente = $request->id_nomina_gape_cliente;
            $idNominaGapeEmpresa = $request->id_nomina_gape_empresa;
    ...
    private function totalAsimilados_04(Request $request)
    {
        try {
            $idNominaGapeCliente = $request->id_nomina_gape_cliente;
            $idNominaGapeEmpresa = $request->id_nomina_gape_empresa;
