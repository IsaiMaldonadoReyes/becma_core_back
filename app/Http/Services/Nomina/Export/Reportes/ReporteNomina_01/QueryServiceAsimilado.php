...
class QueryServiceAsimilado
{
    ...
    public function getData(string $queryName, array $request)
    {
        if (!method_exists($this, $queryName)) {
            throw new \Exception("La consulta '$queryName' no está definida en QueryServiceAsimilado.");
        }
        ...
        try {
            ...
            $sql = "
                ...
                MovimientosSumaQ2 AS (
                    SELECT
                        emp.codigoempleado,
                        ISNULL(CASE WHEN movP.tipoconcepto = ''N'' THEN movP.importetotal ELSE 0 END, 0) AS sueldo,
                        pdo.descripcion,
                ...
                IncidenciasNormalizadasQ2 AS (
                    ...
                    WHERE
                        emp.TipoRegimen IN (''05'', ''06'', ''07'', ''08'', ''09'', ''10'', ''11'')
                ), ' + '
