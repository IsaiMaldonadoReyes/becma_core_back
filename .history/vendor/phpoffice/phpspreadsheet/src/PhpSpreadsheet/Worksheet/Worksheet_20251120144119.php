...
class Worksheet
{
    ...
    /**
     * Inserta nuevas columnas DESPUÉS de una columna dada.
     * Ejemplo: insertNewColumnAfter('C', 2) → inserta 2 columnas después de la C.
     */
    public function insertNewColumnAfter(string $afterColumn, int $numberOfColumns = 1)
    {
        // Convertimos letra a índice numérico
        $colIndex = Coordinate::columnIndexFromString($afterColumn);

        if ($colIndex >= 1) {

            // Convertimos índice → letra de la siguiente columna
            $insertIndex = $colIndex + 1;
            $insertColumnLetter = Coordinate::stringFromColumnIndex($insertIndex);

            $referenceHelper = ReferenceHelper::getInstance();

            // Posición de inserción, ej: "D1" si afterColumn = "C"
            $cellReference = $insertColumnLetter . '1';

            // Insertar columnas
            // insertNewBefore(string $cell, int $columns, int $rows, Worksheet $sheet)
            $referenceHelper->insertNewBefore($cellReference, $numberOfColumns, 0, $this);
        } else {
            throw new Exception('Column must be at least A.');
        }
    ...
    public function insertNewColumnBefore(string $beforeColumn, int $numberOfColumns = 1)
    {
        // Convertimos letra a índice numérico
        $columnIndex = Coordinate::columnIndexFromString($beforeColumn);

        if ($columnIndex >= 1) {
            $referenceHelper = ReferenceHelper::getInstance();

            // Ejemplo: insertar antes de 'C' = 'C1'
            $cellReference = $beforeColumn . '1';

            // Insertar columnas (nueva operación)
            // insertNewBefore(string $beforeCell, int $columns, int $rows, Worksheet $sheet)
            $referenceHelper->insertNewBefore($cellReference, $numberOfColumns, 0, $this);
        } else {
            throw new Exception('Columns can only be inserted before at least column A.');
        }
