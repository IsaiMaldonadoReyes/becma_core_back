...
44class RptPresupuestoController extends Controller
{
    /**
    ...
    public function presupuestoIndividual(Request $request)
    {
        ...
        $queryAgenteComercial = ($request->agente == null ? "" : " AND doc.CIDAGENTE = $request->agente  ");
        $queryAgentePresupuesto = ($request->agente == null ? "" : " AND id_agente = $request->agente  ");

        try {

            $query = DB::connection('sqlsrv_dynamic')->select("
            ...
            $presupuestos = DB::connection('sqlsrv')->select("
                ...
                WHERE 
                    ...
                    $queryAgentePresupuesto
                    AND id_ejercicio = $request->id_ejercicio
                    AND marca = '$idMarca' ");

            $resultados = array_merge($query, $presupuestos);
