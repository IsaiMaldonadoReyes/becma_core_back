...
class RptPresupuestoController extends Controller
{
    ...
    public function presupuestoIndividual(Request $request)
    {
        $idEmpresaUsuario = 1;
        $idEmpresaDatabase =  $request->empresa;
        ...
        $mes = strtolower($request->periodo);
        $anio = $request->ejercicio ?? date('Y');
        ...
        $idMarca = $request->marcas;
        $fechaInicial = Carbon::create($anio, $meses[$mes], 1)->startOfMonth()->format('Y-m-d');
        $fechaFinal = Carbon::create($anio, $meses[$mes], 1)->endOfMonth()->format('Y-m-d');

        $queryAgenteComercial = ($request->idAgente == null ? "" : " AND doc.CIDAGENTE = $request->idAgente  ");
        $queryAgentePresupuesto = ($request->idAgente == null ? "" : " AND id_agente = $request->idAgente  ");
        ...
        try {
            ...
            $query = "
                    ...
                    SELECT 
                        0 AS periodo
                        , ISNULL(SUM(mov.CTOTAL),0) AS TotalVentas
                        , 'Ventas' AS tipo
                FROM [$datosEmpresa->base_datos].dbo.admDocumentos AS doc 
                INNER JOIN [$datosEmpresa->base_datos].dbo.admMovimientos AS mov ON doc.CIDDOCUMENTO = mov.CIDDOCUMENTO 
                INNER JOIN [$datosEmpresa->base_datos].dbo.admProductos AS pro ON mov.CIDPRODUCTO = pro.CIDPRODUCTO 
                INNER JOIN [$datosEmpresa->base_datos].dbo.admClasificacionesValores AS claVal ON pro.CIDVALORCLASIFICACION1 = claVal.CIDVALORCLASIFICACION 
                INNER JOIN [$datosEmpresa->base_datos].dbo.admConceptos AS con ON doc.CIDCONCEPTODOCUMENTO = con.CIDCONCEPTODOCUMENTO 
                WHERE 
                    con.CIDDOCUMENTODE = 4 
                    AND doc.CCANCELADO = 0 
                    $queryAgenteComercial
                    AND doc.CFECHA BETWEEN  CONVERT(VARCHAR, @startDate, 23) 
                                                    AND  CONVERT(VARCHAR, @endDate, 23) 
                    AND claVal.CVALORCLASIFICACION = '$request->marca'
