    ...
    public function index()
    {
        try {
            ...
            for ($i = 0; $i < $num_columnas; $i++) {
                ...
                $tipo_nativo = $meta['native_type']; // Aquí obtienes el tipo SQL (INT, VARCHAR, DECIMAL, etc.)

                // Define el tipo de formato para PhpSpreadsheet
                if ($tipo_nativo === 'DECIMAL' || $tipo_nativo === 'FLOAT' || $tipo_nativo === 'MONEY') {
                    $formatos_columnas[$nombre_columna] = NumberFormat::FORMAT_CURRENCY_USD_SIMPLE;
                } elseif ($tipo_nativo === 'INT' || $tipo_nativo === 'INTEGER') {
                    $formatos_columnas[$nombre_columna] = NumberFormat::FORMAT_NUMBER;
                } else {
                    $formatos_columnas[$nombre_columna] = NumberFormat::FORMAT_GENERAL;
                }
            }
