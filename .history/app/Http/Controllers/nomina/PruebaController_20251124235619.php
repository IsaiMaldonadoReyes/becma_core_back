...
class PruebaController extends Controller
{
    ...
    public function exportPrenomina(Request $request)
    {
        ...
        // ... (tu código de inicialización del spreadsheet) ...

        // 1. Habilitar ajuste de texto para la fila 11 (preferiblemente antes de escribir los datos)
        $sheet->getStyle('A11:CC11')->getAlignment()->setWrapText(true);

        // 2. Asegúrate de que los datos ya estén en las celdas de la fila 11 y otras filas...

        // 3. Iterar sobre las columnas para aplicar el autoajuste de ANCHO
        // NOTA: Es crucial hacer esto DESPUÉS de que todos los datos estén escritos.
        foreach ($sheet->getColumnIterator() as $column) {
            $columnIndex = $column->getColumnIndex();
            // Esto calcula el ancho automáticamente
            $sheet->getColumnDimension($columnIndex)->setAutoSize(true);
        }

        // 4. Indicar que la altura de la fila 11 debe ser automática
        // Esto se debe hacer DESPUÉS de que el ancho de las columnas se haya calculado, 
        // ya que la altura depende del ancho disponible.
        $sheet->getRowDimension(11)->setRowHeight(-1);


        // 5. La línea $sheet->calculateColumnWidths(); DEBE ELIMINARSE.
        // Esta función no es parte de la API pública y no hará nada, o podría causar un error.

        // ... (tu código para guardar el archivo, ej: $writer->save('archivo.xlsx');) ...
