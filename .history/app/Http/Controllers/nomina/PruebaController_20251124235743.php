...
class PruebaController extends Controller
{
    ...
    public function exportPrenomina(Request $request)
    {
        ...
        // 1. Habilitar ajuste de texto para la fila 11 (los títulos)
        $sheet->getStyle('A11:CC11')->getAlignment()->setWrapText(true);

        // 2. Asegúrate de que los datos de los títulos ya estén escritos...

        // 3. Iterar sobre las columnas para aplicar el autoajuste de ANCHO, 
        //    PERO OMITE la columna específica que quieres forzar a un ancho fijo (ej. Columna A)
        foreach ($sheet->getColumnIterator() as $column) {
            $columnIndex = $column->getColumnIndex();

            // **AQUÍ ESTÁ EL CAMBIO CLAVE:**
            // Si quieres que la columna 'A' tenga un ancho fijo y fuerce el salto de línea:
            if ($columnIndex === 'A') {
                // Define un ancho fijo para la columna A (ej. 20 unidades)
                $sheet->getColumnDimension($columnIndex)->setWidth(20);
            } else {
                // Para el resto de columnas, aplica el autoajuste normal
                $sheet->getColumnDimension($columnIndex)->setAutoSize(true);
            }
        }

        // 4. Indicar que la altura de la fila 11 debe ser automática
        //    Esto es necesario para que la fila crezca verticalmente cuando el texto salte de línea.
