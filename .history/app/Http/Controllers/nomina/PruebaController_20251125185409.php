...
class PruebaController extends Controller
{
    ...
    public function exportPrenomina(Request $request)
    {
        ...
        $colInicio = 'I';
        $startIndex = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString($colInicio);


        $start = Coordinate::columnIndexFromString("I");   // 9
        $end   = Coordinate::columnIndexFromString($xlColumnaFinCargaSocial);  // 81 aprox

        for ($i = $start; $i <= $end; $i++) {
            $colLetter = Coordinate::stringFromColumnIndex($i);


            $totalRow = $xlFilaFinDatos; // debajo de los datos
            $colIndex  = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString($colLetter);

            // ⛔ Saltar columnas ANTES de la inicial
            ...
            $sheet->getStyle("{$colLetter}{$xlFilaFinDatos}")->getBorders()->getBottom()->setBorderStyle(
                \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_DOUBLE
            );

            $sheet->getStyle("{$colLetter}{$xlFilaFinDatos}")->getBorders()->getTop()->setBorderStyle(
                \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_DOUBLE
            );
        }
