...
class PruebaController extends Controller
{
    ...
    public function exportPrenomina(Request $request)
    {
        ...
        // 🔥 ELIMINAR TODOS LOS ESTILOS DE COLUMNA (CAUSA REAL DEL PROBLEMA)
        $columnIterator = $sheet->getColumnIterator();
        foreach ($columnIterator as $column) {
            $colIndex = $column->getColumnIndex();
            $sheet->getStyle($colIndex)->getNumberFormat()->setFormatCode('@'); // limpiar
        }
    ...
    public function limpiarMatriz($matriz)
    {
        $invalidValues = [
            '',
            ' ',
        ...
        return array_map(function ($fila) use ($invalidValues) {
            ...
            return array_map(function ($valor) use ($invalidValues) {
                ...
                // Numérico válido
                if (is_numeric($valor)) {
                    return $valor + 0; // cast elegante
                }

                // Si no es numérico → dejar tal cual (texto válido)
                return $valor;
            }, $fila);
        }, $matriz);
    }
