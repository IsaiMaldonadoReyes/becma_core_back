...
class PruebaController extends Controller
{
    public function exportExcel(Request $request)
    {
        ...
        // Columnas de inicio y fin de secciones
        $xlColumnaInicioSeccionNomina = 'I';
        $xlColumnaFinSeccionNomina = '';
        ...
        foreach ($dynamicKeys as $key) {
            ...
            $sheet->setCellValue($col . $baseRow, $key);
            $colIndex++;
        }
         ...
         * ======================================================= */
        $this->aplicarFormatoDinamico($sheet, $baseRow, $startRow, $endRow);




        /** ----------------------------------------
         * 1. Cargar los dos JSON
         * ----------------------------------------
         */
        $jsonFijos = storage_path('app/public/plantillas/datos_fijos.json');
        $empleadosFijos = json_decode(file_get_contents($jsonFijos), true);

        $jsonNominas = storage_path('app/public/plantillas/datos_nominas.json');
        $empleadosNominas = json_decode(file_get_contents($jsonNominas), true);

        /** ----------------------------------------
         * 2. Índices de columnas fijas (A–H)
         * ----------------------------------------
         */
        $fixedKeys = [
            'id',
        ...
        /** ----------------------------------------
         * 3. Detectar columnas dinámicas del pivote SQL
         * ----------------------------------------
         */
        $allKeys = array_keys($empleadosNominas[0]);
        $dynamicKeys = array_diff($allKeys, ['empleado']); // lo único fijo es el ID

        /** ----------------------------------------
         * 4. Insertar columnas dinámicas en Excel
         * ----------------------------------------
         */
        $startDynamicColumn = 'R';
        $numDynamic = count($dynamicKeys);
        ...
        /** ----------------------------------------
         * 5. Poner encabezados de columnas dinámicas
         * ----------------------------------------
         */
        $startIndex = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString($startDynamicColumn);
        ...
        foreach ($dynamicKeys as $key) {
            $column = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($currentIndex);
            $sheet->setCellValue($column . '11', $key);
            $currentIndex++;
        }

        /** ----------------------------------------
         * 6. Generar la matriz final (fijos + dinámicos)
         * ----------------------------------------
         */
        $matriz = [];
        ...
        foreach ($empleadosFijos as $emp) {
            ...
            // Buscar su registro dinámico por ID
            $empNomina = collect($empleadosNominas)->firstWhere('empleado', $emp['nombre']);
        ...
        $sheet->insertNewRowBefore($baseRow + 1, count($matriz) - 1);

        /** ----------------------------------------
         * 8. Copiar formato celda × columna a todas las filas nuevas
         * ----------------------------------------
         */
        $lastColumnIndex = $startIndex + $numDynamic - 1;
        $lastColumnLetter = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($lastColumnIndex);

        /*$this->copiarEstiloPorColumna(
            $sheet,
            $baseRow,
            $startRow,
            $endRow,
    ...
    public function aplicarFormatoDinamico($sheet, $headerRow, $startRow, $endRow)
    {
        ...
        for ($col = 1; $col <= $highestColumnIndex; $col++) {
            ...
            $headerValue = strtolower((string) $sheet->getCell($colLetter . $headerRow)->getValue());

            // Si no hay encabezado → ignorar
            if (trim($headerValue) === '') {
                continue;
            }
             ...
             * -------------------------- */
            if (str_contains($headerValue, 'cantidad')) {
                $sheet->getStyle($range)->getNumberFormat()
                    ->setFormatCode('#,##0');  // Número entero
            }

            /** ------------------------
             *  FORMATO PARA “Monto”
             * ------------------------ */
            if (str_contains($headerValue, 'monto')) {
                $sheet->getStyle($range)->getNumberFormat()
