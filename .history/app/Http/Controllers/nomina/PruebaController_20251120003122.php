...
class PruebaController extends Controller
{
    public function exportExcel(Request $request)
    {
        ...
        // 2. Leer JSON
        $jsonPath = storage_path('app/public/plantillas/datos_fijos.json');
        ...
        foreach ($empleados as $row) {
            $matriz[] = [
                ...
                $row['nombre'],
                $row['puesto'],
                $row['fechaDeIngresoCliente'],
                $row['fechaDeIngresoGape'],
                $row['nss'],
                $row['rfc'],
                $row['curp'],
            ];
        ...
        $ultimaFila = 10 + count($matriz) - 1;

        // 1. Copiar estilos de la fila 10 a todo el rango dinámico
        $sheet->duplicateStyle($sheet->getStyle("A10:H10"), "A10:H{$ultimaFila}");

        // 2. Insertar datos masivamente
        $sheet->fromArray($matriz, null, "A10");
