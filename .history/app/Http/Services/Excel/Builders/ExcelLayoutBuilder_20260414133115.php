    ...
    public function apply($sheet, array $cellsConfig)
    {
        foreach ($cellsConfig as $column) {
            ...
            if (!empty($column['validacion']['comentarioTexto'])) {
                ...
                // 🔥 auto tamaño
                $this->autoSizeComment($comment, $texto);
            }
        }
    }

    private function autoSizeCommentBalanced($comment, string $text)
    {
        // Normalizar saltos
        $text = str_replace('\n', "\n", $text);

        // Quitar markdown de negritas para cálculo
        $plainText = preg_replace('/\*\*(.*?)\*\*/', '$1', $text);

        // 🔥 Limitar ancho (forzar wrap)
        $wrappedText = wordwrap($plainText, 40, "\n"); // 40 caracteres por línea
        ...
        // 📏 Ajustes
        $width = max(140, $maxLength * 5);   // límite ancho
        $width = min($width, 250);           // 🔥 NO muy ancho

        $height = max(80, $lineCount * 18);  // crece vertical
