...
class ExcelLayoutBuilder
{
    ...
    private function autoSizeCommentVertical($comment, string $text)
    {
        // Normalizar saltos
        $text = str_replace('\n', "\n", $text);
        ...
        $width = max(120, $maxLength * 4);   // más angosto
        $height = max(100, $lineCount * 20); // más alto

        // 👉 Garantizar que sea más alto que ancho
        if ($height <= $width) {
            $height = $width + 40;
        }
